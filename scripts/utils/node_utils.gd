...
static func with_when_ready(node: Node, callable: Callable) -> Node:
	if !node.is_node_ready(): node.ready.connect(callable.call(node))
	else: callable.call(node)
	return node

static func with_group(node: Node, group: StringName) -> Node:
